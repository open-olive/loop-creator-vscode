(window["webpackJsonp"] = window["webpackJsonp"] || []).push([[2], {
  ...
  1264: function(module, __webpack_exports__, __webpack_require__) {
    "use strict";
    ...
    const LoginScreen = () => {
      const handleGetCookies = (payload) => __async(void 0, null, function* () {
        try {
          const {url} = payload;
          const {
            remote: {session}
          } = require("electron");
          const cookies = yield session.defaultSession.cookies.get({
            name: "sidekick-jwt",
            url
          });
          yield Object(messaging["E"])(cookies.map(({
            domain,
            expirationDate,
            httpOnly,
            name: cName,
            ...
            domain,
            expires: new Date(expirationDate * 1e3).toISOString(),
            httpOnly,
            name: cName,
            path,
            sameSite: sameSiteLookup[sameSite],
            secure,
            value
          })));
        } catch (err) {
      ...
      const onReady = () => {
        Object(messaging["u"])("getCookies", (payload) => {
          handleGetCookies(payload).catch(console.error);
        }).catch(console.error);
      };
