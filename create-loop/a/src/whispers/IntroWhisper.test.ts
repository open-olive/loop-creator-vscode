...
describe('Intro Whisper', () => {
  ...
  it('creates components with functional handlers', async () => {
    ...
    boxFirstButton.onClick(undefined, newWhisper.whisper as whisper.Whisper);
    expect(console.log).toBeCalledWith('Button 1 clicked.');
    boxSecondButton.onClick(undefined, newWhisper.whisper as whisper.Whisper);
    ...
    textInput.onChange(undefined, 'test', newWhisper.whisper as whisper.Whisper);
    expect(console.log).toBeCalledWith('Text changed: ', 'test');

    // Check email's onChange
    const email = components[9] as whisper.Email;
    email.onChange(undefined, 'test', newWhisper.whisper as whisper.Whisper);
    ...
    password.onChange(undefined, 'test', newWhisper.whisper as whisper.Whisper);
    ...
    telephone.onChange(undefined, 'test', newWhisper.whisper as whisper.Whisper);
    ...
    numberInput.onChange(undefined, 0, newWhisper.whisper as whisper.Whisper);
    expect(console.log).toBeCalledWith('Number changed: ', 0);

    // Check select's onSelect
    const select = components[17] as whisper.Select;
    select.onSelect(undefined, 0, newWhisper.whisper as whisper.Whisper);
    expect(console.log).toBeCalledWith('Selected: ', 0);

    // Check radioBtn's onSelect
    const radioBtn = components[18] as whisper.RadioGroup;
    radioBtn.onSelect(undefined, 0, newWhisper.whisper as whisper.Whisper);
    expect(console.log).toBeCalledWith('Radio button option selected: ', 0);

    // Check updatableMessageInput's onSelect
    const updatableMessageInput = components[30] as whisper.TextInput;
    updatableMessageInput.onChange(undefined, 'test message', newWhisper.whisper as whisper.Whisper);
    ...
    const updatableLabelInput = components[31] as whisper.TextInput;
    updatableLabelInput.onChange(undefined, 'test label', newWhisper.whisper as whisper.Whisper);
    ...
    const autocompleteSingleSelect = components[19] as whisper.Autocomplete;
    autocompleteSingleSelect.onChange?.(undefined, '4', newWhisper.whisper as whisper.Whisper);
    expect(console.log).toBeCalledWith('Autocomplete section changed: ', '4');

    // Check autocomplete's onSelect
    const autocompleteMultipleSelect = components[20] as whisper.Autocomplete;
    autocompleteMultipleSelect.onSelect(undefined, ['4', '5'], newWhisper.whisper as whisper.Whisper);
    expect(console.log).toBeCalledWith('Received selected value: ["4","5"]');

    // Check dateTimeInput's onSelect
    const dateTimeInput = components[21] as whisper.DateTimeInput;
    dateTimeInput.onChange(undefined, '0', newWhisper.whisper as whisper.Whisper);
    expect(console.log).toBeCalledWith('Time picker value received: ', '0');

    // Check icon's onClick
    const icon = components[23] as whisper.Icon;
    icon.onClick?.(undefined, newWhisper.whisper as whisper.Whisper);
    expect(console.log).toBeCalledWith('Emoji Emotions Clicked');

    // Check clicking on a clone button twice
    const cloneButton = components[33] as whisper.Button;
    cloneButton.onClick(undefined, newWhisper.whisper as whisper.Whisper);
    ...
    const resetClonesButton = components[32] as whisper.Button;
    resetClonesButton.onClick(undefined, newWhisper.whisper as whisper.Whisper);
